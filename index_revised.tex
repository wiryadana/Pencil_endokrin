...
\urlstyle{same}
\hypersetup{
  pdftitle={Usia dan Penyakit Ginjal Kronis sebagai Prediktor Dominan High-Risk Plaque pada Pasien Diabetes Mellitus: Evaluasi Peran Indeks TyHbA1c dan Indeks Massa Tubuh dalam Pendekatan Nomogram Klinis},
...
\begin{document}
\title{Usia dan Penyakit Ginjal Kronis sebagai Prediktor Dominan
High-Risk Plaque pada Pasien Diabetes Mellitus: Evaluasi Peran Indeks
TyHbA1c dan Indeks Massa Tubuh dalam Pendekatan Nomogram Klinis}
...
prediktif Usia, Indeks Massa Tubuh (IMT), Indeks TyHbA1c, dan status
Penyakit Ginjal Kronis (PGK Stadium \(\ge\) 3) terhadap keberadaan HRP
serta beban kalsifikasi koroner moderat-berat (CAC \(\ge\) 100
Agatston), sekaligus mengembangkan pendekatan stratifikasi risiko
melalui nomogram klinis. Menggunakan desain potong lintang
...
Januari hingga 31 Desember 2025, total 52 subjek dianalisis. Model
regresi logistik multivariat menunjukkan diskriminasi yang cukup baik
dalam memprediksi HRP (\emph{Area Under the Curve} {[}AUC{]} = 0,724)
maupun kalsifikasi CAC \(\ge\) 100 (AUC = 0,782). Usia secara konsisten
menjadi prediktor independen yang bermakna secara statistik pada kedua
model (HRP: OR 1,08; \(p\) = 0,050 dan CAC \(\ge\) 100: OR 1,10; \(p\) =
0,021). Status PGK menunjukkan tren peningkatan risiko HRP yang relevan
secara klinis (OR 2,24) namun belum mencapai kemaknaan statistik pada
ukuran sampel ini (\(p\) = 0,368), sementara Indeks TyHbA1c dan IMT
tidak menunjukkan kontribusi independen yang bermakna terhadap kedua
luaran tersebut. Berdasarkan temuan ini, disusun nomogram klinis
komprehensif (memuat seluruh parameter kardiometabolik untuk kepentingan
eksplorasi patofisiologis) serta nomogram tereduksi berbasis Usia dan
status PGK yang lebih ringkas untuk aplikasi klinis praktis.
Simpulannya, pada kohort tunggal berukuran terbatas ini, Usia dan status
PGK---bukan Indeks TyHbA1c atau IMT---tampil sebagai prediktor dominan
kerentanan plak (HRP) sekaligus beban kalsifikasi koroner; peran Indeks
TyHbA1c sebagai penanda kardiometabolik independen memerlukan validasi
lebih lanjut pada kohort yang lebih besar sebelum dapat direkomendasikan
sebagai instrumen skoring klinis mandiri.
...
ability of Age, Body Mass Index (BMI), the TyHbA1c Index, and Chronic
Kidney Disease status (CKD Stage \(\ge\) 3) for HRP presence and
moderate-to-severe coronary artery calcification burden (CAC \(\ge\) 100
Agatston), and to develop a clinical nomogram-based risk stratification
approach. Using a cross-sectional design among prediabetes and DM
patients undergoing CTCA at RSUP Ngoerah from January 1 to December 31,
2025, a total of 52 subjects were analyzed. Multivariate logistic
regression showed reasonably good discrimination for predicting HRP
(Area Under the Curve {[}AUC{]} = 0.724) and CAC \(\ge\) 100 (AUC =
0.782). Age was consistently a statistically significant independent
predictor in both models (HRP: OR 1.08, p = 0.050; CAC \(\ge\) 100: OR
1.10, p = 0.021). CKD status showed a clinically relevant increase in
HRP risk (OR 2.24) that did not reach statistical significance at this
sample size (p = 0.368), while the TyHbA1c Index and BMI showed no
significant independent contribution to either outcome. Based on these
findings, a comprehensive nomogram (including all cardiometabolic
parameters, for pathophysiological exploration) and a reduced nomogram
based on Age and CKD status (a more parsimonious tool for practical
clinical use) were developed. In conclusion, in this limited
single-center cohort, Age and CKD status---rather than the TyHbA1c Index
or BMI---emerged as the dominant predictors of both plaque vulnerability
(HRP) and coronary calcification burden; the independent value of the
TyHbA1c Index as a cardiometabolic marker requires further validation in
larger cohorts before it can be recommended as a standalone clinical
scoring instrument.
\end{abstract}
...
personal secara visual. Mengingat keterbatasan ukuran sampel, seluruh
model multivariat divalidasi secara internal menggunakan teknik
\emph{bootstrap resampling} (B = 200 replikasi) untuk memperoleh
estimasi \emph{C-index} dan kurva kalibrasi yang terkoreksi optimisme
(\emph{optimism-corrected}), sementara interval kepercayaan 95\% untuk
nilai AUC dihitung menggunakan metode DeLong. Sebagai analisis
sensitivitas tambahan, hubungan Indeks TyHbA1c terhadap beban
kalsifikasi koroner turut dievaluasi dengan skor CAC sebagai variabel
kontinu (transformasi logaritma), guna meminimalkan hilangnya informasi
statistik akibat dikotomisasi pada ambang CAC \(\ge\) 100.
...
\pandocbounded{\includegraphics[keepaspectratio]{index_revised_files/figure-pdf/fig-roc-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_revised_files/figure-pdf/fig-nomogram-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_revised_files/figure-pdf/fig-nomogram-reduksi-1.pdf}}
...
\textbf{Validasi Internal Model HRP (Bootstrap Resampling)}

Guna mengoreksi potensi \emph{overfitting} yang umum terjadi pada ukuran
sampel terbatas (N = 52), validasi internal menggunakan teknik
\emph{bootstrap resampling} (B = 200 iterasi) dilakukan untuk mengukur
performa diskriminatif yang sesungguhnya (\emph{optimism-corrected
C-index}).

Berdasarkan hasil perhitungan validasi ditemukan \textbf{Model
Komprehensif (4 Variabel)} Menunjukkan nilai \emph{C-index apparent}
(kemampuan diskriminasi pada data latih) sebesar 0.724. Namun, setelah
dikoreksi terhadap optimisme model (0.082), performa aktualnya turun
menjadi \textbf{0.642}. \textbf{Model Tereduksi (Usia + PGK):}
Menunjukkan nilai \emph{C-index apparent} sebesar 0.724, dan terkoreksi
menjadi \textbf{0.702}. Tingkat optimisme pada model ini jauh lebih
rendah (hanya 0.022).

Penurunan performa (\emph{optimisme}) yang jauh lebih kecil pada model
tereduksi membuktikan bahwa model yang lebih \emph{streamline} ini lebih
stabil dan lebih tangguh (\emph{robust}) saat diaplikasikan pada
populasi baru. Lebih lanjut, Uji Rasio Kemungkinan (\emph{Likelihood
Ratio Test} / LRT) memvalidasi bahwa penambahan IMT dan Indeks TyHbA1c
ke dalam model dasar (Usia + PGK) tidak memberikan perbaikan akurasi
model yang bermakna secara statistik (\(p\) = 0.993). Visualisasi
kesesuaian antara probabilitas prediksi dengan observasi aktual untuk
model komprehensif ditunjukkan pada kurva kalibrasi di bawah ini
(@fig-kalibrasi-hrp).
...
menghasilkan diskriminasi diagnostik yang baik untuk memprediksi CAC
\(\ge\) 100. Analisis ROC menunjukkan nilai \emph{Area Under the Curve}
(AUC) sebesar 0.782 (78.2\%), seperti yang ditampilkan pada
...
\pandocbounded{\includegraphics[keepaspectratio]{index_revised_files/figure-pdf/fig-roc-cac-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_revised_files/figure-pdf/fig-nomogram-cac-1.pdf}}
...
\textbf{Validasi Internal Model CAC (Bootstrap Resampling)}

Sama halnya dengan pemodelan HRP, model prediksi kuantitas kalsifikasi
(CAC \$\textbackslash ge\$ 100) juga divalidasi secara internal
menggunakan *bootstrap resampling* (B = 200).

Hasil evaluasi menunjukkan bahwa model ini memiliki \emph{C-index
apparent} sebesar 0.782 (95\% CI: 0.645--0.918). Setelah dilakukan
iterasi \emph{bootstrap}, performa diskriminatif model ini terkoreksi
secara realistis menjadi \textbf{0.74} dengan tingkat optimisme yang
tergolong kecil (0.042).

Uji Rasio Kemungkinan (LRT) antara model dasar (Usia + PGK) dengan model
yang ditambah Indeks TyHbA1c menunjukkan bahwa penambahan parameter
metabolik tersebut tidak memperbaiki \emph{goodness-of-fit} pemodelan
kalsifikasi secara signifikan (\(p\) = 0.173). Kesesuaian performa
prediksi model kalsifikasi terhadap observasi klinis aktual
divisualisasikan secara grafis melalui kurva kalibrasi
(@fig-kalibrasi-cac).

\begin{verbatim}

n=52   Mean absolute error=0.071   Mean squared error=0.0072
...
\pandocbounded{\includegraphics[keepaspectratio]{index_revised_files/figure-pdf/fig-kalibrasi-cac-1.pdf}}

}

\caption{\label{fig-kalibrasi-cac}\textbf{Kurva Kalibrasi Model CAC
\(\ge\) 100 (Bootstrap-Corrected, B = 200).} Garis diagonal putus-putus
merepresentasikan kalibrasi sempurna; garis biru merepresentasikan
performa aktual model setelah koreksi optimisme.}

\end{figure}%

Estimasi \emph{C-index} terkoreksi-optimisme untuk model CAC (0.74)
dibandingkan dengan nilai \emph{apparent} (0.782; 95\% CI 0.645--0.918)
memberikan estimasi performa yang lebih konservatif dan realistis. Uji
rasio kemungkinan antara model dasar (Usia + PGK) dan model dengan
tambahan Indeks TyHbA1c menghasilkan nilai \(p\) = 0.173.

\textbf{Analisis Sensitivitas: Indeks TyHbA1c terhadap CAC sebagai
Variabel Kontinu}

Dikotomisasi skor CAC pada ambang \(\ge\) 100 berpotensi menghilangkan
informasi statistik, khususnya pada ukuran sampel yang terbatas. Sebagai
analisis sensitivitas, hubungan Indeks TyHbA1c dengan beban kalsifikasi
koroner turut dievaluasi menggunakan skor CAC sebagai variabel kontinu
(ditransformasi logaritma natural, \(\ln(\text{CAC}+1)\), mengingat
distribusinya yang sangat menjulur ke kanan).

\begin{verbatim}

    Shapiro-Wilk normality test

data:  df_cac_cont$TyHbA1c_Index
W = 0.9815, p-value = 0.5917
\end{verbatim}
...
data:  df_cac_cont$log_CAC
W = 0.8707, p-value = 4.339e-05
\end{verbatim}
...
data:  df_cac_cont$TyHbA1c_Index and df_cac_cont$Global_CAC_score
S = 26204, p-value = 0.4025
...
data:  df_cac_cont$TyHbA1c_Index and df_cac_cont$log_CAC
t = -1.8051, df = 50, p-value = 0.07708
alternative hypothesis: true correlation is not equal to 0
95 percent confidence interval:
 -0.48735499  0.02739734
sample estimates:
       cor 
-0.2473524 
\end{verbatim}
...
Call:
lm(formula = log_CAC ~ TyHbA1c_Index + Usia + CKD_Status, data = df_cac_cont)

Residuals:
    Min      1Q  Median      3Q     Max 
-5.4511 -1.1802  0.2324  1.4132  4.4581 

Coefficients:
              Estimate Std. Error t value Pr(>|t|)  
(Intercept)    3.30318    4.73096   0.698    0.488  
TyHbA1c_Index -0.54455    0.52694  -1.033    0.307  
Usia           0.08336    0.03522   2.367    0.022 *
CKD_Status     0.68712    0.74094   0.927    0.358  
---
Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1

Residual standard error: 2.298 on 48 degrees of freedom
Multiple R-squared:  0.1951,    Adjusted R-squared:  0.1448 
F-statistic: 3.879 on 3 and 48 DF,  p-value: 0.0146
\end{verbatim}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_revised_files/figure-pdf/fig-scatter-tyhba1c-cac-1.pdf}}

}

\caption{\label{fig-scatter-tyhba1c-cac}\textbf{Hubungan Indeks TyHbA1c
dengan Global CAC Score (skala logaritmik).} Setiap titik
merepresentasikan satu subjek; garis biru adalah garis regresi linear
dengan pita 95\% CI.}

\end{figure}%

Hasil korelasi Spearman antara Indeks TyHbA1c dan skor CAC kontinu
menunjukkan koefisien \(\rho\) = -0.119 (\(p\) = 0.402). Pada model
regresi linear yang disesuaikan terhadap Usia dan status PGK, koefisien
Indeks TyHbA1c terhadap \(\ln(\text{CAC}+1)\) adalah -0.545 (\(p\) =
0.307). Konsistensi (atau inkonsistensi) arah dan kemaknaan hasil ini
dengan model regresi logistik biner pada CAC \(\ge\) 100 perlu
didiskusikan secara eksplisit: apabila kedua pendekatan sama-sama
menunjukkan hasil non-signifikan, hal ini memperkuat kesimpulan bahwa
Indeks TyHbA1c tidak memiliki hubungan independen yang kuat dengan beban
kalsifikasi pada kohort ini --- bukan sekadar artefak dikotomisasi.
...
\textbf{Paradoks Kalsifikasi dan Pengaruh Terapi Statin}

Analisis sensitivitas pada studi ini mengungkap tren korelasi yang
berbanding terbalik (negatif) antara Indeks TyHbA1c dan skor CAC absolut
(Spearman \(\rho\) = -0.119; koefisien OLS = -0.545), meskipun tidak
mencapai signifikansi statistik. Secara klinis, fenomena ini dapat
dijelaskan melalui morfologi plak: lesi dengan kerentanan tinggi (HRP)
yang dipicu oleh badai glukolipotoksisitas seringkali didominasi oleh
inti lipid nekrotik masif (\emph{low-attenuation plaque}) dan
kalsifikasi tipe bercak (\emph{spotty calcification}) yang tidak
terdeteksi kuat oleh perangkat lunak skoring Agatston konvensional.
Lebih lanjut, mayoritas pasien (71.2\%) dalam kohort ini memiliki
riwayat penggunaan statin. Terapi statin jangka panjang diketahui
menginduksi stabilisasi plak melalui makrokalsifikasi. Hal ini dapat
bertindak sebagai perancu patofisiologis (\emph{confounder}), di mana
pasien dengan faktor risiko tinggi yang diobati secara agresif justru
akan menampilkan skor CAC yang memburuk, sehingga berpotensi mendilusi
nilai prediktif murni dari parameter metabolik seperti TyHbA1c terhadap
kuantitas kalsium.
...
Kombinasi parameter klinis dan kardiometabolik (Usia, IMT, Indeks
TyHbA1c, dan status PGK Stadium \(\ge\) 3) terbukti memiliki kemampuan
diskriminasi diagnostik yang solid. Studi ini memperkenalkan inovasi
pendekatan ``Stratifikasi Ganda'' melalui pengembangan tiga nomogram
klinis aplikatif. Nomogram pertama (komprehensif dan tereduksi)
dirancang secara spesifik memprediksi kerentanan kualitas plak
(\emph{High-Risk Plaque} / HRP) dengan \emph{C-index}
terkoreksi-optimisme masing-masing sebesar 0.642 dan 0.702. Sementara
itu, nomogram sekunder memprediksi kuantitas beban kalsifikasi
moderat-berat (CAC \(\ge\) 100) dengan akurasi yang lebih stabil
(\emph{C-index} terkoreksi 0.74) yang sangat didominasi oleh faktor
...
non-invasif, dengan menyadari bahwa glukolipotoksisitas menggerakkan
kerentanan struktural plak, sedangkan usia dan durasi penyakit menyetir
beban kalsifikasi anatomis.
