...
df_clean <- df_raw %>%
  ...
  mutate(
    ...
    # TAMBAHAN: Variabel Terapi dan Durasi DM
...
pct_statin <- round((n_statin / n_total) * 100, 1)
med_cac_statin <- round(median(df_clean$Global_CAC_score[df_clean$Statin == "On-Statin"], na.rm=TRUE), 1)
med_cac_nostatin <- round(median(df_clean$Global_CAC_score[df_clean$Statin == "No-Statin"], na.rm=TRUE), 1)
n_hrp_statin <- sum(df_clean$Statin == "On-Statin" & df_clean$HRP_Status == "HRP (+)", na.rm=TRUE)
pct_hrp_statin <- round((n_hrp_statin / n_statin) * 100, 1)
n_nostatin <- sum(df_clean$Statin == "No-Statin", na.rm=TRUE)
n_hrp_nostatin <- sum(df_clean$Statin == "No-Statin" & df_clean$HRP_Status == "HRP (+)", na.rm=TRUE)
pct_hrp_nostatin <- round((n_hrp_nostatin / n_nostatin) * 100, 1)
p_cac_statin <- round(wilcox.test(df_clean$Global_CAC_score ~ df_clean$Statin)$p.value, 3)
p_hrp_statin <- round(chisq.test(table(df_clean$Statin, df_clean$HRP_Status))$p.value, 3)

# 4. Statistik Terapi DM
...
med_cac_diet <- round(median(df_clean$Global_CAC_score[df_clean$Terapi_DM == "Tanpa Terapi/Diet"], na.rm=TRUE), 1)
med_cac_oad <- round(median(df_clean$Global_CAC_score[df_clean$Terapi_DM == "OAD"], na.rm=TRUE), 1)
med_cac_ins <- round(median(df_clean$Global_CAC_score[df_clean$Terapi_DM == "Insulin"], na.rm=TRUE), 1)
pct_hrp_diet <- round((sum(df_clean$Terapi_DM == "Tanpa Terapi/Diet" & df_clean$HRP_Status == "HRP (+)", na.rm=TRUE) / n_diet) * 100, 1)
pct_hrp_oad <- round((sum(df_clean$Terapi_DM == "OAD" & df_clean$HRP_Status == "HRP (+)", na.rm=TRUE) / n_oad) * 100, 1)
pct_hrp_ins <- round((sum(df_clean$Terapi_DM == "Insulin" & df_clean$HRP_Status == "HRP (+)", na.rm=TRUE) / n_ins) * 100, 1)
p_cac_terapi <- round(kruskal.test(Global_CAC_score ~ Terapi_DM, data = df_clean[!is.na(df_clean$Terapi_DM), ])$p.value, 3)
p_hrp_terapi <- round(chisq.test(table(df_clean$Terapi_DM, df_clean$HRP_Status))$p.value, 3)

# 5. Statistik Riwayat DM
n_baru <- sum(df_clean$Riwayat_DM == "Baru Terdiagnosis", na.rm=TRUE)
n_lama <- sum(df_clean$Riwayat_DM == "Sudah Lama", na.rm=TRUE)
med_cac_baru <- round(median(df_clean$Global_CAC_score[df_clean$Riwayat_DM == "Baru Terdiagnosis"], na.rm=TRUE), 1)
med_cac_lama <- round(median(df_clean$Global_CAC_score[df_clean$Riwayat_DM == "Sudah Lama"], na.rm=TRUE), 1)
pct_hrp_baru <- round((sum(df_clean$Riwayat_DM == "Baru Terdiagnosis" & df_clean$HRP_Status == "HRP (+)", na.rm=TRUE) / n_baru) * 100, 1)
pct_hrp_lama <- round((sum(df_clean$Riwayat_DM == "Sudah Lama" & df_clean$HRP_Status == "HRP (+)", na.rm=TRUE) / n_lama) * 100, 1)
p_cac_lama <- round(wilcox.test(Global_CAC_score ~ Riwayat_DM, data = df_clean[!is.na(df_clean$Riwayat_DM), ])$p.value, 3)
p_hrp_lama <- round(fisher.test(table(df_clean$Riwayat_DM, df_clean$HRP_Status))$p.value, 3)
```
...
**Evaluasi Terapi Statin terhadap Karakteristik Plak**

Mengingat mayoritas pasien dalam kohort ini (`r n_statin` pasien, `r pct_statin`%) telah menggunakan terapi statin saat dirujuk untuk pemeriksaan CTCA, analisis komparatif dilakukan untuk melihat efek terapi terhadap arsitektur plak. Pasien yang menerima statin (*On-Statin*) mencatatkan median skor kalsifikasi yang lebih tinggi (`r med_cac_statin` Agatston) dibandingkan kelompok *No-Statin* (`r med_cac_nostatin` Agatston), kendati secara statistik tidak bermakna ($p$ = `r p_cac_statin`). Terkait kerentanan plak, kelompok *On-Statin* justru mencatatkan proporsi penemuan HRP yang sedikit lebih tinggi (`r pct_hrp_statin`%) dibandingkan kelompok *No-Statin* (`r pct_hrp_nostatin`%), meskipun perbedaan ini juga tidak signifikan secara statistik ($p$ = `r p_hrp_statin`).

**Evaluasi Modalitas Terapi dan Durasi Penyakit**

Analisis lebih lanjut dilakukan terhadap efek jenis terapi penurun glukosa dan durasi penyakit terhadap karakteristik plak. Pasien yang mendapat terapi OAD mencatatkan median kalsifikasi (CAC) tertinggi (`r med_cac_oad` Agatston) dibandingkan pengguna Insulin (`r med_cac_ins` Agatston) dan pasien tanpa terapi spesifik/diet (`r med_cac_diet` Agatston), meskipun secara statistik tidak berbeda bermakna ($p$ = `r p_cac_terapi`). Sebaliknya, proporsi HRP justru ditemukan paling tinggi pada kelompok tanpa terapi spesifik (`r pct_hrp_diet`%) dibandingkan kelompok Insulin (`r pct_hrp_ins`%) maupun OAD (`r pct_hrp_oad`%) ($p$ = `r p_hrp_terapi`).

Terkait durasi penyakit, pasien yang telah lama terdiagnosis DM menunjukkan beban kalsifikasi yang jauh lebih masif (`r med_cac_lama` Agatston) dibandingkan pasien yang baru terdiagnosis (`r med_cac_baru` Agatston), kendati belum bermakna secara statistik pada sampel ini ($p$ = `r p_cac_lama`). Menariknya, proporsi kerentanan plak (HRP) antara pasien lama (`r pct_hrp_lama`%) dan baru terdiagnosis (`r pct_hrp_baru`%) tampak sangat serupa ($p$ = `r p_hrp_lama`).
...
#| fig-cap: "**Hubungan Indeks TyHbA1c dengan Global CAC Score (skala logaritmik).** Setiap titik merepresentasikan satu subjek; garis biru adalah garis regresi linear dengan pita 95% CI. Ringkasan uji normalitas dan korelasi dicantumkan di dalam grafik."
#| echo: false
#| warning: false
#| message: false
#| fig-width: 7
...
# Memformat angka p-value agar rapi (misal: jika < 0.001 tidak ditulis 0.000)
p_shap_ty <- ifelse(shapiro_ty$p.value < 0.001, "< 0.001", paste0("= ", round(shapiro_ty$p.value, 3)))
p_shap_cac <- ifelse(shapiro_logcac$p.value < 0.001, "< 0.001", paste0("= ", round(shapiro_logcac$p.value, 3)))

# Merangkai teks anotasi yang akan ditempel ke dalam grafik
teks_anotasi <- paste0(
  "Uji Normalitas (Shapiro-Wilk):\n",
  "- Indeks TyHbA1c: p ", p_shap_ty, "\n",
  "- ln(CAC+1): p ", p_shap_cac, "\n\n",
  "Korelasi (Spearman):\n",
  "- rho = ", round(cor_spearman_cac$estimate, 3), "  (p = ", round(cor_spearman_cac$p.value, 3), ")"
)
...
ggplot(df_cac_cont, aes(x = TyHbA1c_Index, y = log_CAC)) +
  ...
  # Menambahkan teks anotasi di pojok kanan atas grafik
  annotate("text", x = Inf, y = Inf, label = teks_anotasi, 
           hjust = 1.05, vjust = 1.1, size = 3.5, color = "#333333") +
  labs(
...
**Analisis Jalur (*Path Analysis*): Efek Mediasi Poros Kardiorenal**

Untuk menguji hipotesis patofisiologis mengenai poros kardiorenal, *path analysis* dieksekusi secara paralel pada kedua luaran utama studi ini (HRP dan CAC $\ge$ 100) menggunakan pendekatan *Structural Equation Modeling* (SEM) [@kline2015principles]. Usia ditetapkan sebagai variabel independen, status PGK sebagai mediator, dan luaran aterosklerotik sebagai variabel dependen.
...
# 1. Model Mediasi untuk HRP
model_med_hrp <- '
  CKD_Status ~ a * Usia
  HRP_Status ~ b * CKD_Status + c * Usia
  indirect_effect := a * b
  total_effect := c + (a * b)
'
fit_med_hrp <- sem(model_med_hrp, data = df_clean, ordered = c("CKD_Status", "HRP_Status"))
summary_med_hrp <- parameterEstimates(fit_med_hrp)

p_path_a <- round(summary_med_hrp$pvalue[summary_med_hrp$label == "a"], 3)
p_path_b <- round(summary_med_hrp$pvalue[summary_med_hrp$label == "b"], 3)
p_path_c <- round(summary_med_hrp$pvalue[summary_med_hrp$label == "c"], 3)
p_indirect <- round(summary_med_hrp$pvalue[summary_med_hrp$label == "indirect_effect"], 3)

# 2. Model Mediasi untuk CAC >= 100
model_med_cac <- '
  CKD_Status ~ a2 * Usia
  CAC_100_plus ~ b2 * CKD_Status + c2 * Usia
  indirect_effect2 := a2 * b2
  total_effect2 := c2 + (a2 * b2)
'
fit_med_cac <- sem(model_med_cac, data = df_clean, ordered = c("CKD_Status", "CAC_100_plus"))
summary_med_cac <- parameterEstimates(fit_med_cac)

p_path_a2 <- round(summary_med_cac$pvalue[summary_med_cac$label == "a2"], 3)
p_path_b2 <- round(summary_med_cac$pvalue[summary_med_cac$label == "b2"], 3)
p_path_c2 <- round(summary_med_cac$pvalue[summary_med_cac$label == "c2"], 3)
p_indirect2 <- round(summary_med_cac$pvalue[summary_med_cac$label == "indirect_effect2"], 3)
```

Hasil analisis jalur pada domain kualitas plak mengonfirmasi bahwa Usia memiliki efek langsung yang bermakna secara statistik terhadap kemunculan HRP (Jalur C, $p$ = `r p_path_c`). Terkait efek mediasi, penuaan menunjukkan tren korelasi positif terhadap penurunan fungsi ginjal/PGK (Jalur A, $p$ = `r p_path_a`), namun efek lanjutan dari PGK menuju HRP pada model ini tidak mencapai signifikansi statistik (Jalur B, $p$ = `r p_path_b`). Secara keseluruhan, efek tidak langsung (*indirect effect*) dari Usia $\rightarrow$ PGK $\rightarrow$ HRP belum terbukti bermakna ($p$ = `r p_indirect`).

Pola yang lebih ekstrem terlihat pada domain kuantitas kalsifikasi (CAC $\ge$ 100). Dominansi Usia sebagai prediktor langsung kalsifikasi koroner tampak sangat absolut ($p$ = `r p_path_c2`), sedangkan jalur mediasi dari komplikasi PGK menuju pembentukan kalsium sama sekali tidak bermakna ($p$ = `r p_path_b2`), mengonfirmasi ketiadaan efek mediasi tidak langsung pada ekuasi ini ($p$ = `r p_indirect2`). Visualisasi diagram struktural untuk kedua model disajikan pada \@fig-path-diagram dan \@fig-path-diagram-cac.

```{r}
#| label: fig-path-diagram
#| fig-cap: "**Diagram Analisis Jalur untuk *High-Risk Plaque* (HRP).** Angka pada garis panah merepresentasikan nilai estimasi koefisien jalur."
#| echo: false
#| warning: false
#| message: false
#| fig-width: 7
...
semPaths(fit_med_hrp, 
         what = "paths", 
         whatLabels = "est",        
         layout = "tree",           
         rotation = 2, 
         nCharNodes = 0,            # Mencegah nama variabel disingkat (Usi -> Usia)
         intercepts = FALSE,        # Menghilangkan panah segitiga yang mengganggu
         residuals = FALSE,         # Menghilangkan panah melingkar putus-putus
         sizeMan = 12,              # Menyesuaikan lebar kotak
         sizeMan2 = 6,              # Menyesuaikan tinggi kotak
         edge.label.cex = 1.1,      
         edge.label.position = 0.5, # Memastikan angka tepat di tengah garis panah
         color = "#FFE0E0",         
         edge.color = "#D32F2F",    
         label.cex = 1.2,           
         fade = FALSE,              
...
```{r}
#| label: fig-path-diagram-cac
#| fig-cap: "**Diagram Analisis Jalur untuk Kalsifikasi Koroner (CAC $\\ge$ 100).** Bukti visual dominansi Usia secara langsung terhadap skor kalsifikasi absolut."
#| echo: false
#| warning: false
#| message: false
#| fig-width: 7
...
semPaths(fit_med_cac, 
         what = "paths", 
         whatLabels = "est",        
         layout = "tree",           
         rotation = 2,              
         nCharNodes = 0,            # Mencegah nama variabel disingkat
         intercepts = FALSE,        # Menghilangkan panah segitiga
         residuals = FALSE,         # Menghilangkan panah melingkar
         sizeMan = 12,              
         sizeMan2 = 6,              
         edge.label.cex = 1.1,      
...
**Peran Dominan Penuaan, Durasi Paparan, dan Poros Kardiorenal**

Dalam seluruh pemodelan, usia muncul sebagai penentu utama. Proses aterosklerosis pada dasarnya adalah patologi kumulatif yang bergantung pada waktu (*time-dependent*). Dominansi efek penuaan dan paparan ini diperkuat oleh temuan analisis durasi penyakit, di mana pasien yang telah lama terdiagnosis DM mencatatkan kalsifikasi anatomis nyaris tiga kali lipat lebih tebal dibandingkan pasien *newly diagnosed*.

Menariknya, proporsi plak rentan (HRP) pada pasien baru terdiagnosis pun sudah sangat tinggi. Hal ini secara fundamental menyoroti konsep memori metabolik (*metabolic memory*), di mana *atherosclerotic remodeling* yang mengancam jiwa sejatinya telah berlangsung secara diam-diam sejak fase prediabetes, jauh sebelum diagnosis klinis DM ditegakkan.

Secara patofisiologis, kehadiran komplikasi Penyakit Ginjal Kronis (PGK Stadium $\ge$ 3) dihipotesiskan bertindak sebagai katalisator tambahan melalui poros kardiorenal (*cardiorenal syndrome*), di mana retensi toksin uremik berinteraksi sinergis memicu degradasi stabilitas koroner. Namun, meskipun konsep poros kardiorenal ini sangat kokoh secara teoretis, analisis jalur (*path analysis*) pada studi ini belum dapat membuktikan efek mediasi PGK secara statistik. Efek langsung (*direct effect*) dari penuaan terhadap kerusakan vaskular tampak terlalu absolut dan dominan, sehingga efek tidak langsung (*indirect effect*) melalui perantara PGK menjadi tersamar (*diluted*) akibat keterbatasan *power* statistik dari ukuran sampel kohort ini.

**Paradoks Kalsifikasi dan Indikasi Bias Modalitas Terapi**

Analisis sensitivitas pada studi ini mengungkap tren korelasi yang berbanding terbalik (negatif) antara Indeks TyHbA1c dan skor CAC absolut (Spearman $\rho$ = `r round(cor_spearman_cac$estimate, 3)`; koefisien OLS = `r round(coef(model_lm_cac_cont)["TyHbA1c_Index"], 3)`). Fenomena ini dapat dijelaskan melalui morfologi plak: lesi dengan kerentanan tinggi (HRP) yang dipicu oleh badai glukolipotoksisitas didominasi oleh inti lipid nekrotik masif (*low-attenuation plaque*) yang tidak terdeteksi kuat oleh perangkat lunak skoring Agatston konvensional.

Lebih lanjut, pertimbangan *indication bias* tidak dapat dihindari, di mana mayoritas pasien (`r pct_statin`%) telah menggunakan terapi statin. Terapi statin jangka panjang diketahui menginduksi stabilisasi plak melalui proses makrokalsifikasi, yang terlihat dari tingginya kecenderungan skor CAC pada pengguna statin dibanding kelompok tanpa terapi statin. Pola serupa terlihat pada modalitas terapi DM: pengguna OAD menunjukkan skor kalsifikasi tertinggi, sedangkan proporsi plak rentan (HRP) justru memuncak pada kelompok asimptomatik tanpa intervensi farmakologis (hanya diet). Hal ini mengindikasikan bahwa pasien asimptomatik yang belum terpapar efek stabilisasi pleiotropik dari medikasi konvensional seringkali masih berada pada fase inflamasi vaskular yang sangat aktif (HRP tinggi).
